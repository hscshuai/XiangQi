// 将/帅的移动规则
const generalRule = (fromPos, toPos) => {
  // 只能在九宫格内移动
...
// 士/仕的移动规则
const advisorRule = (fromPos, toPos) => {
  // 只能在九宫格内移动
  if (toPos.x < 3 || toPos.x > 5) return false;
  if (fromPos.y <= 4 && toPos.y > 4) return false;
  if (fromPos.y > 4 && toPos.y <= 4) return false;
...
// 象/相的移动规则
const elephantRule = (fromPos, toPos, board) => {
  // 不能过河
  if (fromPos.y <= 4 && toPos.y > 4) return false;
  if (fromPos.y > 4 && toPos.y <= 4) return false;
...
// 马的移动规则
const horseRule = (fromPos, toPos, board) => {
...
// 车的移动规则
const chariotRule = (fromPos, toPos, board) => {
...
// 炮的移动规则
const cannonRule = (fromPos, toPos, board) => {
  if (fromPos.x !== toPos.x && fromPos.y !== toPos.y) return false;
...
// 兵/卒的移动规则
const pawnRule = (fromPos, toPos, isRed) => {
  const forward = isRed ? -1 : 1;
  
  // 未过河只能向前
  if ((isRed && fromPos.y > 4) || (!isRed && fromPos.y <= 4)) {
...
// 导出统一的规则检查函数
export const checkMoveRule = (pieceType, fromPos, toPos, board, isRed) => {
  switch (pieceType.toLowerCase()) {
    case 'general':
      return generalRule(fromPos, toPos);
    case 'advisor':
      return advisorRule(fromPos, toPos);
    case 'elephant':
      return elephantRule(fromPos, toPos, board);
    case 'horse':
      return horseRule(fromPos, toPos, board);
    case 'chariot':
      return chariotRule(fromPos, toPos, board);
    case 'cannon':
      return cannonRule(fromPos, toPos, board);
    case 'pawn':
      return pawnRule(fromPos, toPos, isRed);
    default:
      return false;
  }
};
